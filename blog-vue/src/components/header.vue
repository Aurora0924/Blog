...
<div class="">
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/head_bg.jpg)'}">
		<div class="scene">
    	...
    	<img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'static/img/tou.png'" alt="">
      	...
      	{{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"欢迎来到Augenstern-Zhou的个人博客"}}
      </h2>
		</div>
	</div>
</div>
...
export default {
	data() { //选项 / 数据
		return {
			...
			// path:'',//当前打开页面的路径
			input: '', //input输入内容
			headBg: 'url(static/img/head_bg.jpg)', //头部背景图
			headTou: '', //头像
			projectList: '' //项目列表
		}
	},
